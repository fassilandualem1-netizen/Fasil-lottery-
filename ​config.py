...
def deduct_balance_safely(user_id: str, amount: float, game_mode: str = "real") -> str:
    """
    Race Condition እንዳይከሰት በ Lua Script አቶሚክ በሆነ መንገድ ሂሳብ ይቀንሳል።
    """
    ...
    """
    try:
        # Upstash Redis eval ሲያደርግ የሚቀበለው KEYS እና ARGV በዝርዝር (List) መልክ ነው
...
def add_to_history(user_id: str, entry: dict):
    """
    የተጠቃሚዎችን የታሪክ መዝገብ በRedis List ውስጥ ያስቀምጣል፤ ርዝመቱን በ20 ይገድባል።
    """
    try:
...
def update_history_tx_status(user_id: str, tx_id: str, status: str):
    """
    የፔንዲንግ ትራንዛክሽን ሁኔታ ሲቀየር (ለምሳሌ Deposit ሲፈቀድ) የታሪክ ሁኔታውን ያዘምናል።
    """
    try:
        history_key = f"history:{user_id}"
        ...
        for item_raw in raw_history:
            # Upstash redis በቀጥታ string ስለሚመልስ decode ማድረግ አያስፈልግም
        ...
        if updated and new_history:
            ...
            # የነበረውን ቅደም ተከተል ጠብቆ ለመመለስ reversed እንጠቀማለን
