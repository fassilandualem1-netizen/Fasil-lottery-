...
def welcome(m):
    ...
    bot.send_message(uid, welcome_msg, reply_markup=main_kb, parse_mode="Markdown")
...
def show_boards(m):
    ...
    # አክቲቭ የሆኑ ሰሌዳዎችን ብቻ ማሳያ
    found_board = False
    for k, v in data["boards"].items():
        if v["active"]:
            kb.add(telebot.types.InlineKeyboardButton(
                f"🎰 {v['name']} - ({v['price']} ETB)", 
                callback_data=f"start_sel_{k}"
            ))
            found_board = True
    ...
    if found_board:
        ...
        bot.send_message(uid, "⚠️ ለጊዜው ክፍት የሆኑ ሰሌዳዎች የሉም።")
...
def handle_calls(c):
    ...
    if c.data.startswith("start_sel_"):
        ...
        text = (
            f"✅ **{b['name']} ተመርጧል!**\n"
            f"💰 **የአንድ እጣ ዋጋ፦** `{b['price']} ETB` \n\n"
            f"🏦 **የባንክ አካውንቶች፦**\n"
            f"🔸 CBE: `1000584461757` \n"
            f"🔸 Telebirr: `0951381356` \n"
            f"━━━━━━━━━━━━━\n"
            f"📩 እባክዎ ብሩን ከላኩ በኋላ **ደረሰኙን (ፎቶ)** እዚህ ይላኩ።"
        )
        bot.edit_message_text(text, uid, c.message.message_id, parse_mode="Markdown")
