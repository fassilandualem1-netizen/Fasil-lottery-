...
def run():
    # Koyeb የሚሰጠውን PORT በራሱ እንዲቀበል ተደርጓል
...
def load_data():
    ...
    try:
        ...
        if res.data:
            ...
            print("✅ የድሮ ዳታ ከ Supabase ተጭኗል!")
    except Exception as e:
        print(f"❌ Load Error: {e}")

# ዳታውን ወደ Supabase ሴቭ ማድረጊያ
def save_data():
    try:
        supabase.table("bot_data").upsert({"id": "main_db", "content": data}).execute()
        # ለተጨማሪ ጥንቃቄ ወደ ቻናልህም ባክአፕ ይልካል
...
# --- 4. ያንተ ዋና ዋና ተግባራት (Logic) ---
# እዚህ በታች ያለው ያንተ ምርጥ ኮድ ነው፣ ምንም ንክኪ የለውም

def get_user(uid, name="ደንበኛ"):
    uid = str(uid)
    if uid not in data["users"]:
        data["users"][uid] = {"name": name, "wallet": 0}
        save_data() # አዲስ ተጠቃሚ ሲመጣ ወዲያው ሴቭ ያደርጋል
