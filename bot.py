...
def welcome(message):
    ...
    welcome_text = (
        ...
        "🇪🇹 <b>FASIL VIP BINGO</b> - የታመነ የቢንጎ መጫወቻ ቦት!\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "💳 <b>የአካውንት ገለፃ፦</b>\n"
...
def update_value(message, b_id, action):
    try:
        ...
        save_data(); bot.send_message(ADMIN_ID, "✅ መረጃው ተቀይሯል!")
    except: bot.send_message(ADMIN_ID, "⚠️ ስህተት ተፈጥሯል!")
...
def finalize_registration(message, b_id, name):
    ...
    try:
        ...
        if num in board["slots"] or int(num) < 1 or int(num) > board["max"]:
            bot.send_message(uid, "❌ <b>ቁጥሩ ተይዟል ወይም ከሰሌዳው ውጭ ነው።</b> እባክዎ ሌላ ይሞክሩ።")
            return
