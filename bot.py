...
def customer_view_categories(call):
    ...
    # በሃይል ሁሉንም ምድቦች እንዘረዝራለን (Emojis ን እዚህ እናጥፋቸው ለcallback ደህንነት)
    categories_map = {
        "ምግብ ቤት": "🍴ምግብ ቤት",
        "ሱፐርማርኬት": "🛍️ ሱፐርማርኬት",
        "መጠጥ": "🍹መጠጥ",
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    for callback_val, display_name in categories_map.items():
        markup.add(types.InlineKeyboardButton(display_name, callback_data=f"cat_select_{callback_val}"))

    markup.add(types.InlineKeyboardButton("🏠 ወደ ዋናው ሜኑ", callback_data="go_to_customer_menu"))
    
    bot.edit_message_text(
        "📍 **የንግድ ዘርፍ ይምረጡ**\n\nድርጅቶችን ለማየት ምድብ ይምረጡ፦", 
...
def list_vendors_by_category(call):
    ...
    for v_id, v_info in vendors.items():
        # 🛠 FORCE CLEANUP: የቬንደሩን መረጃ አጽድቶ ማመሳሰል
        v_raw_cat = str(v_info.get('category', ''))
        # በሃይል ምልክቶችን፣ ኢሞጂዎችን እና ቅንፎችን ማጥፋት
        v_clean_cat = re.sub(r"[\[\]'🍴🛍️🍹💊💄👕\s]", "", v_raw_cat).strip()
        target_cat = re.sub(r"[🍴🛍️🍹💊💄👕\s]", "", selected_cat).strip()
        
        if v_clean_cat == target_cat:
            is_open = v_info.get('is_open', True)
            status_icon = "🟢" if is_open else "🔴"
            btn_text = f"{status_icon} {v_info.get('name', 'ድርጅት')}"
            
            # 🛠 FORCE ID: ID-ው በሃይል String መሆኑን ማረጋገጥ
            c_data = f"view_vendor_prods_{str(v_id).strip()}"
            markup.add(types.InlineKeyboardButton(btn_text, callback_data=c_data))
            found_any = True

    if not found_any:
        return bot.answer_callback_query(call.id, f"⚠️ በ'{selected_cat}' ዘርፍ በአሁኑ ሰዓት ድርጅት የለም።", show_alert=True)

    markup.add(types.InlineKeyboardButton("🔙 ወደ ምድቦች ተመለስ", callback_data="cust_view_shops"))
    bot.edit_message_text(f"🏢 **የ'{selected_cat}' ድርጅቶች**", 
...
def get_flexible_unit(category_name):
    """
    በንግድ ዘርፉ መሰረት ትክክለኛውን መለኪያ በሃይል ይመርጣል
    """
    cat = str(category_name).strip()
    # ኢሞጂዎችን እና ክፍት ቦታዎችን በሃይል ማጽዳት
    clean_cat = re.sub(r"[🍴🛍️🍹💊🚨👕\s]", "", cat)
    
    if "ምግብ" in clean_cat:
        return "ቁጥር"
    elif "ሱፐርማርኬት" in clean_cat:
        return "KG"
    elif "መጠጥ" in clean_cat:
        return "ፍሬ"
    elif any(x in clean_cat for x in ["ፋርማሲ", "ኮስሞቲክስ", "ቡቲክ"]):
        return "ፍሬ"
    else:
        return "ፍሬ"
...
def process_add_to_cart(call):
    ...
    try:
        ...
        return bot.answer_callback_query(call.id, "⚠️ ስህተት ተፈጥሯል፣ እባክዎ ደግመው ይሞክሩ።", show_alert=True)
    ...
    product = db.get('products', {}).get(p_id)
    if not product:
        return bot.answer_callback_query(call.id, "⚠️ እቃው በዳታቤዝ ውስጥ አልተገኘም።", show_alert=True)
    ...
    if user_cart['vendor_id'] and user_cart['vendor_id'] != p_vendor_id:
        return bot.answer_callback_query(call.id, "❌ መጀመሪያ በቅርጫትዎ ያለውን ድርጅት ትዕዛዝ ያጠናቅቁ ወይም ያጥፉ።", show_alert=True)
    ...
    if not existing_item and len(user_cart['items']) >= 3:
        return bot.answer_callback_query(call.id, "❌ በሳይክል ስለሚመላለስ በአንድ ጊዜ ቢበዛ 3 አይነት እቃ ብቻ ይቻላል።", show_alert=True)

    # 🛠 FORCE 3: የ 5 KG አጠቃላይ ክብደት ገደብ
    current_weight = sum(float(i.get('weight', 0.5)) * i['quantity'] for i in user_cart['items'])
    new_total_weight = current_weight + (p_weight * qty)
    
    if new_total_weight > 5.0:
        return bot.answer_callback_query(call.id, f"⚖️ የክብደት ገደብ አልፏል! አጠቃላይ ክብደት ከ 5KG መብለጥ የለበትም። (አሁን: {new_total_weight:.1f}kg)", show_alert=True)
    ...
    if existing_item:
        ...
        user_cart['items'].append({
            "id": p_id,
            "name": product['name'],
            "price": p_price, # እዚህ ጋር p_price ተስተካክሏል
            "quantity": qty,
            "weight": p_weight
        })
    ...
    # ወደ ንዑስ ምድብ ይመልሰዋል
    view_vendor_sub_categories(call) 
...
def generate_checkout_bill(user_id, u_lat, u_lon):
    ...
    if not v_id or not cart.get('items'):
        return "🛒 ቅርጫትዎ ባዶ ነው!", 0

    vendor_info = db.get('vendors_list', {}).get(v_id, {})
    admin_settings = db.get('admin_settings', {})

    # 1. የእቃዎቹ ድምር
    items_total = sum(float(i['price']) * i['quantity'] for i in cart.get('items', []))
    
    # 2. የዴሊቨሪ ክፍያ (በሃይል)
    v_lat = vendor_info.get('lat')
    v_lon = vendor_info.get('lon')
    delivery_fee, dist_detail = calculate_dynamic_delivery_fee(u_lat, u_lon, v_lat, v_lon)

    # 3. የአድሚን ክፍያዎች (Fixed Numbers)
    service_fee = float(admin_settings.get('service_fee_amount', 0)) if admin_settings.get('service_fee_on') else 0
    extra_time_fee = float(admin_settings.get('time_fee_amount', 0)) if admin_settings.get('time_fee_on') else 0
...
def send_welcome(message):
    ...
    # 4. ደንበኛ ከሆነ (ከላይ ያሉት ካልሆኑ)
    else:
        users = db.get('users', {})
        # ደንበኛው ቀድሞ ተመዝግቦ ስልክ ቁጥር እና ሎኬሽን (lat) ካለው
        if user_id_str in users and users[user_id_str].get('phone') and users[user_id_str].get('lat'):
            markup = get_customer_dashboard()
            bot.send_message(user_id, "🔥 **እንኳን ደህና መጡ! ምን ማዘዝ ይፈልጋሉ?**", reply_markup=markup, parse_mode="Markdown")
        else:
            # ምዝገባ ካልጨረሰ መጀመሪያ ስልክ እንዲልክ እንጠይቃለን
            markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
            markup.add(types.KeyboardButton("📱 ስልክ ቁጥር ለመላክ እዚህ ይጫኑ", request_contact=True))
            
            bot.send_message(user_id, "👋 እንኳን ወደ BDF በሰላም መጡ!\n\nለመቀጠል እባክዎ መጀመሪያ ስልክ ቁጥርዎን ያጋሩ።", reply_markup=markup)
...
def list_only_vendors(call):
    try:
        db = load_data()
        vendors = db.get('vendors_list', {})

        report = "🏢 **የተመዘገቡ ድርጅቶች (Vendors)**\n"
        report += "━━━━━━━━━━━━━━━━━━━━\n\n"

        if not vendors:
            report += "⚠️ እስካሁን ምንም ድርጅት አልተመዘገበም።"
        else:
            for v_id, info in vendors.items():
                cat = info.get('category', 'ምድብ የሌለው')
                if isinstance(cat, (list, tuple)): cat = cat[-1]

                report += (
                    f"🏢 **{info.get('name', 'N/A')}**\n"
                    f"   📞 ስልክ፦ `{info.get('phone', 'N/A')}`\n"
                    f"   📁 ምድብ፦ `{cat}`\n"
                    f"   💰 ባላንስ፦ `{info.get('deposit_balance', 0)}` ብር\n"
                    f"   🆔 ID፦ `{v_id}`\n"
                    f"------------------------\n"
                )
...
def confirm_reset_request(call):
    markup = types.InlineKeyboardMarkup()
    # ቀጣዩን ካልባክ 'admin_final_reset_confirm' ብለን እንጥራው
