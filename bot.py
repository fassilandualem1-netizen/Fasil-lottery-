...
# --- 1. Web Hosting (Railway/Render እንዳይዘጋ) ---
app = Flask('')
@app.route('/')
...
def welcome(message):
    ...
    welcome_text = (
        f"👋 <b>እንኳን ወደ ፋሲል መዝናኛ በደህና መጡ!</b>\n\n"
        f"👤 <b>ስም፦</b> {user['name']}\n"
...
def handle_selection(call):
    ...
    if user["wallet"] < board["price"]:
        bot.answer_callback_query(call.id, "⚠️ በቂ ሂሳብ የሎትም! እባክዎ መጀመሪያ አካውንቶን ይሙሉ።", show_alert=True)
        return
...
def finalize_reg_inline(call, bid, num):
    ...
    bot.edit_message_text(f"🎯 <b>ተሳክቷል!</b>\n🎰 ሰሌዳ፦ {bid}\n🎫 ቁጥር፦ {num}\n💰 ቀሪ ሂሳብ፦ {user['wallet']} ብር", 
                          uid, call.message.message_id, reply_markup=main_menu_markup(uid))
...
def finalize_dec(message, target):
    ...
    bot.send_message(message.chat.id, "✅ መረጃው ለተጠቃሚው ተልኳል።")
...
def handle_receipts(message):
    uid = str(message.chat.id)
    bot.reply_to(message, "⏳ <b>ደረሰኝዎ እየተመረመረ ነው...</b>\nእባክዎ ትንሽ ይጠብቁ።")
    ...
    cap = f"📩 <b>አዲስ ደረሰኝ</b>\n👤 <b>ስም፦</b> {message.from_user.first_name}\n🆔 <b>ID፦</b> <code>{uid}</code>"
