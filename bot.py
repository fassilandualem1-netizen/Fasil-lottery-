...
def process_vendor_fund_amount(message, v_id, v_name):
    try:
        ...
        if amount <= 0:
            bot.send_message(message.chat.id, "⚠️ መጠኑ ከ 0 በላይ መሆን አለበት።")
            return

        db = load_data()
        
        # የቆየውን ሂሳብ ማንበብ (ከሌለ 0)
        old_balance = db['vendors_list'][v_id].get('deposit_balance', 0)
        new_balance = old_balance + amount
        
        # ዳታቤዝ ላይ ማዘመን
        db['vendors_list'][v_id]['deposit_balance'] = new_balance
        
        # 'balance' የሚል ኪይ ካለህ እሱንም ማዘመን ይቻላል
        if 'balance' in db['vendors_list'][v_id]:
            db['vendors_list'][v_id]['balance'] = new_balance
            
        save_data(db)
        
        # ለአድሚኑ ማረጋገጫ መላክ
        success_msg = (f"✅ **በተሳካ ሁኔታ ተሞልቷል!**\n\n"
                       f"🏢 ድርጅት፦ {v_name}\n"
                       f"💵 የተሞላው መጠን፦ `{amount:,.2f} ETB`\n"
                       f"💰 አሁን ያለው ጠቅላላ ቀሪ ሂሳብ፦ `{new_balance:,.2f} ETB`")
        
        bot.send_message(message.chat.id, success_msg, reply_markup=get_admin_dashboard(message.from_user.id))
        
        # ለድርጅቱ (Vendor) ማሳወቂያ መላክ
        try:
            notification = (f"🔔 **የሂሳብ መሙያ ማሳወቂያ**\n\n"
                            f"በአድሚን በኩል `{amount:,.2f} ETB` ሂሳብዎ ላይ ተጨምሯል።\n"
                            f"💰 የአሁኑ ቀሪ ሂሳብዎ፦ `{new_balance:,.2f} ETB` ነው")
            bot.send_message(v_id, notification, parse_mode="HTML")
        except:
            print(f"ለድርጅት {v_id} ማሳወቂያ መላክ አልተቻለም (ቦቱን አቁሞት ሊሆን ይችላል)")
