...
def load_db():
    try:
        # ቦቱ ሲነሳ ዳታቤዙን ለማንበብ 3 ጊዜ ይሞክራል (ለጥንቃቄ)
        for attempt in range(3):
            ...
            time.sleep(2) # ካልተሳካ 2 ሰከንድ ቆይቶ እንደገና ይሞክራል
...
def handle_calls(c):
    ...
    # አድሚኑ ብር እራሱ እንዲጽፍ የሚያደርግ ቁልፍ
        ...
        if bid and data["users"][uid]["wallet"] >= b["price"]:
            if n not in b["slots"]:
                # ብሩን ከዋሌት መቀነስ
                data["users"][uid]["wallet"] -= b["price"]
                b["slots"][n] = {"name": data["users"][uid]["name"], "id": uid}
                
                refresh_group(bid)
                bot.answer_callback_query(c.id, f"✅ ቁጥር {n} ተይዟል!")
                bot.send_message(uid, f"✅ ቁጥር {n} ተይዟል!\n💰 ቀሪ ዋሌት፦ `{data['users'][uid]['wallet']} ETB`")
                # ብር ስለተቀነሰ ወዲያው ሴቭ ማድረግ
                save_db()
            else:
                bot.answer_callback_query(c.id, "⚠️ ይቅርታ፣ ይህ ቁጥር አሁን ተይዟል!", show_alert=True)
        else:
            bot.answer_callback_query(c.id, f"❌ በቂ ሂሳብ የለዎትም! የ1 እጣ ዋጋ {b['price']} ETB ነው።", show_alert=True)
...
if __name__ == "__main__":
    load_db() # መጀመሪያ ዳታ መጫን
    
    # Render የሚሰጠውን ፖርት በራስ ሰር መፈለግ
    ...
    # ፖሊንግ ፍጥነትን ለመጨመር (Non-stop polling)
    while True:
        try:
