...
from config import (
    ...
    register_user, login_user, get_or_create_telegram_user,
    request_password_reset, confirm_password_reset,
    get_all_sports_matches, save_sports_matches
)
...
def register_endpoint():
    try:
        ...
        # Optional: If user is from Telegram
        telegram_id = data.get('telegram_id')
        
        result = register_user(username, password, telegram_id, initial_balance=0.0)
...
@server.route('/api/telegram-login', methods=['POST'])
def telegram_login_endpoint():
    try:
        data = request.get_json()
        telegram_id = data.get('telegram_id')
        ...
        result = get_or_create_telegram_user(telegram_id, user_name, initial_balance=100.0)
        if result["status"] == "success":
            return jsonify(result), 200
        else:
            return jsonify(result), 400
        ...
        return jsonify({"status": "error", "message": "Telegram login failed"}), 500


@server.route('/api/password-reset/request', methods=['POST'])
def password_reset_request_endpoint():
    try:
        ...
        if not username:
            return jsonify({"status": "error", "message": "Please enter your username"}), 400
        
        result = request_password_reset(username)
        return jsonify(result), 200
    except Exception as e:
        print(f"Password Reset Request Error: {e}")
        return jsonify({"status": "error", "message": "Failed to request password reset"}), 500


@server.route('/api/password-reset/confirm', methods=['POST'])
def password_reset_confirm_endpoint():
    try:
        ...
        if not reset_code or not new_password or len(new_password) < 6:
            return jsonify({"status": "error", "message": "Please enter a valid reset code and new password (min 6 characters)"}), 400
...
# 2. የ /admin ኮማንድ (Admin Only)
@bot.message_handler(commands=['admin'])
def send_admin_panel(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        bot.reply_to(message, "❌ እርስዎ አድሚን አይደሉም!")
        return

    # Admin Keyboard
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(
        KeyboardButton("📋 የማታች ዝርዝር /matches"),
        KeyboardButton("➕ አዲስ ማታች /add_match"),
        KeyboardButton("✏️ ኦድ አስተካክል /setodd"),
        KeyboardButton("🗑️ ማታች አጥፍ /delmatch")
    )
    bot.send_message(message.chat.id, "🎮 <b>የአድሚን ማለትዎት</b>\n\nከታች ያለውን ሜኑ ይጠቀሙ:", parse_mode="HTML", reply_markup=markup)
...
def send_matches(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        bot.reply_to(message, "❌ እርስዎ አድሚን አይደሉም!")
        return
    matches = get_all_sports_matches()
    text = "📋 <b>የማታች ዝርዝር</b>\n"
    for i, m in enumerate(matches):
        text += f"\n<b>#{i+1} {m['league']}</b>\n{m['home']} vs {m['away']}\n"
        text += f"1: {m['odds']['1']} | X: {m['odds']['X']} | 2: {m['odds']['2']}\n"
        text += f"1X: {m['odds']['1X']} | X2: {m['odds']['X2']}\n"
        text += f"Active: {'✅' if m['active'] else '❌'}\n"
        text += f"<code>ID: {m['id']}</code>\n"
    bot.send_message(message.chat.id, text, parse_mode="HTML")

# 4. የ /add_match ኮማንድ (simple step-by-step: first just add a demo match for testing, we can make it interactive later)
@bot.message_handler(commands=['add_match'])
def add_match_cmd(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        bot.reply_to(message, "❌ እርስዎ አድሚን አይደሉም!")
        return
    matches = get_all_sports_matches()
    new_match = {
        "id": f"match-{len(matches)+1}-{int(time.time())}",
        "league": "Ethiopian Premier League",
        "home": "Home Team",
        "away": "Away Team",
        "start_time": int(time.time()) + 3600,
        "odds": {"1": 2.0, "X": 3.0, "2": 2.5, "1X": 1.5, "X2": 1.8},
        "active": True
    }
    matches.append(new_match)
    save_sports_matches(matches)
    bot.reply_to(message, f"✅ አዲስ ማታች ተጨምሯል!\nID: {new_match['id']}")

# 5. የ /setodd ኮማንድ (simple: /setodd [match_index] [market] [new_odd], e.g. /setodd 1 1 2.2)
@bot.message_handler(commands=['setodd'])
def set_odd(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        bot.reply_to(message, "❌ እርስዎ አድሚን አይደሉም!")
        return
    try:
        ...
        if len(parts) <4:
            bot.reply_to(message, "❌ ስህተት! አጠቃቀም: /setodd [match_index (1,2..)] [market (1,X,2,1X,X2)] [new_odd]\nእንዲሁም: /setodd 1 1 2.2")
            return
        match_idx = int(parts[1]) -1
        market = parts[2].upper()
        new_odd = float(parts[3])
        matches = get_all_sports_matches()
        if match_idx <0 or match_idx >= len(matches):
            bot.reply_to(message, "❌ የተሳሳተ ማታች ኢንዴክስ!")
            return
        if market not in matches[match_idx]['odds']:
            bot.reply_to(message, f"❌ የተሳሳተ ማርኬት! ለሚቻሉ ማርኬቶች: {list(matches[match_idx]['odds'].keys())}")
            return
        matches[match_idx]['odds'][market] = new_odd
        save_sports_matches(matches)
        bot.reply_to(message, f"✅ ኦድ ተስተካክሏል!\n{matches[match_idx]['home']} vs {matches[match_idx]['away']}\n{market} = {new_odd}")
    except Exception as e:
...
def del_match(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) <2:
            bot.reply_to(message, "❌ ስህተት! አጠቃቀም: /delmatch [match_index (1,2..)]")
            return
        match_idx = int(parts[1]) -1
        matches = get_all_sports_matches()
        if match_idx <0 or match_idx >= len(matches):
            bot.reply_to(message, "❌ የተሳሳተ ማታች ኢንዴክስ!")
            return
        del matches[match_idx]
        save_sports_matches(matches)
        bot.reply_to(message, "✅ ማታች ተሰርዟል!")
    except Exception as e:
        bot.reply_to(message, f"❌ ስህተት ተከሰተ! {str(e)}")

# 7. Original /start ኮማንድ - ንፁህ እና ቀጥተኛ
...
if __name__ == "__main__":
    ...
    # For testing, use regular Flask server
    server.run(host="127.0.0.1", port=int(os.environ.get('PORT', 5001)), debug=False)
